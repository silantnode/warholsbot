...
bot.on( crtvEcon, msg => {
  ...
  connection.query('SELECT * FROM tasks', function( error, rows ){
    ...
    while( randSelectNums.length < 5 ){

        var randNum = Math.ceil( Math.random() * rows.length )
        if( randSelectNums.indexOf( randNum ) > -1 ) continue;
        randSelectNums[ randSelectNums.length ] = randNum;
    ...
    for ( var i = 0; i < ( randSelectNums.length ) ; i++) {
        
        var giftSelector;
        taskListDisplay += '/' + ( i + 1 )+ ' ';
        giftSelector = randSelectNums[i];
