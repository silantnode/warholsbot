...
bot.on( GIFT_ECON, msg =>{
    ...
    return bot.sendMessage( msg.from.id, `${ giftListDisplay }`, { markup } );
...
// Checks if a user has selected content from the provided random list. 
...
bot.on( '/*' , msg => {
  ...
  if ( currentGiftSelection.length == 5 ) {
    ...
    // These need to be used across several sub-routines/
    ...
    if ( giftNumber >= 1 && giftNumber <= 5 ) {
      ...
      GetBalance( msg.from.id, function( error, result ){
        ...
        return bot.sendMessage( msg.from.id, `You now have more warhols. Enjoy!`, { markup });
