...
bot.on( CREATIVE_ECON, msg => {
  ...
  if ( warholMode == 1 ){ // We are in get mode.
    ...
    // Whatever spend functions will be needed.
...
bot.on( GIFT_ECON, msg =>{
    ...
    if ( warholMode == 1) { // We are in get mode... whee!
    ...
    } else if ( warholMode == 2 ) { // We are in spend mode. Yahoooo.

        // Whatever functions are needed.
...
bot.on( SPECULATIVE_ECON, msg => {
    ...
    if ( warholMode == 1 ) {
        ...
        // Whatever code needed for throwing away your warhols with abandon.
...
bot.on( '/*' , msg => {
  ...
  if ( warholMode == 1 ){
    ...
    // Whatever is needed for numerical command responses in spend mode.
